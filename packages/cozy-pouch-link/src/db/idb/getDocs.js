...
const parserIDBDocIdAndRev = doc => {
  const docid_and_rev = doc._doc_id_rev.split('::')
  if (docid_and_rev.lenth < 2) {
    logger.error(`Malformed _doc_id_rev attribute: ${docid_and_rev}`)
    return
  }
  doc._id = docid_and_rev[0]
  doc._rev = docid_and_rev[1]
}
